...
```
setup ──▶ bank ──▶ drill new ──▶ (you answer, offline, on the clock) ──▶ drill grade ──▶ stats
  │         │           │                                                    │             │
exam:    question    sealed paper                                     mistakes become   score curve
block     bank       + key + answer sheet                              flashcards      vs. real exams
```
...
2. **`sisyphus bank <COURSE>`** — ingests your own past-paper PDFs into a topic-tagged question bank (kept outside the vault — exam text is copyrighted).
3. **`drill new <COURSE>`** — a sealed, timed, closed-book paper matching the real exam's point split, weighted toward the topics you marked `invest`. The agent never prints a question into chat: files only. Then you answer it offline, real clock, no notes, no AI.
4. **`drill grade <COURSE> <date>`** — in a fresh session (the one that wrote the key can't invigilate), graded hostilely against a point breakdown fixed at creation time. Every dropped point becomes one atomic flashcard for Obsidian's Spaced Repetition plugin.
5. **`sisyphus stats`** — score curve over time and per-topic breakdown, computed from result frontmatter, with your real exam results interleaved as ground truth.
...
Built July 2026. In daily dogfooding for three VUT FIT (Brno) first-year courses from September 2026; the past-paper path has been exercised only on mock PDFs until real corpora exist. Schemas are stable — the semester's dataset depends on them. Issues and PRs welcome.
