...
Before that gate, log only a plain misconception, evidence pointer and next question in `<exam.corpus>/pending-learning.md` (or a user-chosen private directory outside the vault and repos). No flashcard syntax, answer draft, cloze, or scheduling comments in the pending queue. Do not quote exam questions or keys into concept notes or decks. Abstract the underlying concept in the student's words after the attempt.
