...
2. **Collect results.** In each course's drills dir (default `<Area>/exam-drill/<code>/`, or `exam.drills_dir`), read the frontmatter of every file with `type: drill-result` or `type: exam-result`:
...
3. **Regenerate `<drills_dir>/progress.md`** (overwrite):
   ...
   - **Score over time** — one table row per result, drills and real exams interleaved by date: date · score/total · % · source (`past-paper`/`synthetic`/`mixed`, or `**exam: final**` bolded — real exams are the ground truth the drill curve is judged against).
