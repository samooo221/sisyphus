...
3. **Write `<attempt>-result.md`** — build it in a temporary file and move it into place once complete, the same staging paper creation uses, so an interrupted grading never leaves a half-written result in the drills dir. Per-question notes and one *redo this problem type* line per dropped question, with frontmatter:
   ```yaml
